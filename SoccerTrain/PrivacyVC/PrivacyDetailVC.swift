...
class PrivacyDetailVC: UIViewController, WKNavigationDelegate {
    ...
    init(detailTitle:String, detailUrl:[String]?) {
        super.init(nibName: nil, bundle: nil)
        self.modalPresentationStyle = UIModalPresentationStyle.overCurrentContext
        titleDesccnentt = detailTitle
        if (detailUrl != nil) {
            multiUrlcnentt = detailUrl!
        }
    }
    ...
    func setDetailCallBack(wefke2: String?, wm0240: String?, wkfl230: String?, wlkker: (() -> Void)?) {
        callbackcnentt = wlkker
    }
